...
func findFirstEnglishDigit (input:String) -> Int? {
    var savedIndex:String.Index?
    var savedKey:String = ""
    for key in engDigits.keys {
        if input.contains(key), let currentIndex = input.firstRange(of: key)?.lowerBound {
            if let saved = savedIndex {
...
//            print("first eng digit of input text: " + input[savedIndex!...])
//            print("saved Key: " + savedKey)
        }
    }
    return engDigits[savedKey]
}

func findLastEnglishDigit (input:String) -> Int? {
    var savedIndex:String.Index?
    var savedKey:String = ""
    for key in engDigits.keys {
        if input.contains(key), 
            let currentIndex = input
            .ranges(of: key)
            .sorted(by: { r1, r2 in
...
//            print("last eng digit of input text: " + input[savedIndex!...])
//            print("saved Key: " + savedKey)
...
for line in lines {
    ...
    var curLine = Array(line)
    var firstNum:String = "", SecondNum:String = ""
    var firstIndex:Int = 0, lastIndex:Int = 999
    var firstString:String = "", lastString:String = ""
    
    // locate the first numerical digit
    for i in 0..<line.count{
        if curLine[i].isNumber {
            firstIndex = i
            firstNum = String(curLine[i])
            break
        }
    }
    
    // update the first english digit
    if firstIndex > 2 {
        firstString = String(curLine[0..<firstIndex])
        if let firstNumber = findFirstEnglishDigit(input: firstString) {
            firstNum = String(firstNumber)
            print("first number updated to " + firstNum)
        }
    }
    
    // locate the last numerical digit
    for i in (0..<line.count).reversed(){
        if curLine[i].isNumber{
            lastIndex = i
            SecondNum = String(curLine[i])
            break
        }
    }
    
    // updatethe last english digit
    if curLine.count - lastIndex > 2 {
        lastString = String( curLine[lastIndex+1..<curLine.count])
        if let lastNumber = findLastEnglishDigit(input: lastString) {
            SecondNum = String(lastNumber)
            print("last number updated to " + SecondNum)
        }

    }
    
    
    var final:String = firstNum + "" + SecondNum
    
    print(final)
    if let finalNumber = Int(final) {
